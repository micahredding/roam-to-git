...
class Config:
    def __init__(
        self, database: Optional[str], debug: bool, sleep_duration: float = 2.0
    ):
        ...
        assert (
            self.database
        ), "Please define the Roam database you want to backup."
        self.debug = debug
        self.sleep_duration = sleep_duration


async def download_rr_archive(
    output_type: str,
    ...
    if config.debug:
        ...
        document = pages[0]
        return await _download_rr_archive(
            document, output_type, output_directory, config
        )
...
async def _download_rr_archive(
    document: Page,
    output_type: str,
    output_directory: Path,
    config: Config,
    ...
    for _ in range(333):
        ...
        if strong:
            if "database's you are an admin of" == await get_text(
                document, strong
            ):
                logger.error(
    ...
    (export_all,) = [
        b for b in divs_pb3 if await get_text(document, b) == "export all"
    ]
    await export_all.click()
    await asyncio.sleep(config.sleep_duration)

    async def get_dropdown_button():
        dropdown_button = await document.querySelector(
        ...
        assert dropdown_button_text in [
            "markdown",
            "json",
    ...
    if button_text != output_type:
        ...
        await button.click()
        await asyncio.sleep(config.sleep_duration)
        output_type_elems = await document.querySelectorAll(
            ".bp3-text-overflow-ellipsis"
        )
        (output_type_elem,) = [
            e
            for e in output_type_elems
    ...
    (export_all_confirm,) = [
        b for b in buttons if await get_text(document, b) == "export all"
    ]
...
async def signin(document, config: Config, sleep_duration=1.0):
    """Sign-in into Roam"""
    ...
    buttons = await document.querySelectorAll("button")
    (signin_confirm,) = [
        b for b in buttons if await get_text(document, b) == "sign in"
    ]
...
def scrap(markdown_zip_path: Path, json_zip_path: Path, config: Config):
    ...
    tasks = [
        download_rr_archive("markdown", Path(markdown_zip_path), config=config),
    ...
    if config.debug:
        ...
        logger.warning(
            "Exiting without updating the git repository, "
